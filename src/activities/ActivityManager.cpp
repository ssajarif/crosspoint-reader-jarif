...
void ActivityManager::goToFileBrowser(std::string path, std::string selectedFile) {
  replaceActivity(std::make_unique<FileBrowserActivity>(renderer, mappedInput, std::move(path), std::move(selectedFile)));
}
