...
void BluetoothSettingsActivity::renderMainMenu() {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, "Bluetooth");
...
void BluetoothSettingsActivity::renderDeviceList() {
  ...
  char countStr[32];
  snprintf(countStr, sizeof(countStr), btMgr->isScanning() ? "Scanning..." : "Found %zu", devices.size());
  ...
  const auto labels = mappedInput.mapLabels("Back", "Connect", "Left", "Retry");
  GUI.drawButtonHints(renderer, labels.btn1, labels.btn2, labels.btn3, labels.btn4);
